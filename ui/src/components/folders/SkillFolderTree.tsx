    ...
    <div className="relative hidden h-full shrink-0 md:flex" style={{ width: `${width}px` }}>
      ...
      {loading ? (
        ...
        <div className="space-y-0.5">
          ...
          <RailHeading label={t("Organization")} onCreate={() => onCreateFolder(null)} />
          {model.company.length > 0 ? (
            ...
            <div className="px-2 py-1 text-xs text-muted-foreground">{t("No organization folders yet.")}</div>
          )}
        ...
        {editable || canNest ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {editable ? (
                <>
                  <DropdownMenuItem onSelect={() => onStartRename(folder)}>{t("Rename")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onEditFolder(folder)}>{t("Edit color")}</DropdownMenuItem>
                  <DropdownMenuItem onSelect={() => onMoveFolder(folder, isInMySkills ? "company" : "my")}>
                    <MoveRight className="h-3.5 w-3.5" />
                    {t("Move to")} {isInMySkills ? t("Organization") : t("My Skills")}
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          {isCurrent ? <span className="text-xs text-muted-foreground">{t("current")}</span> : null}
          {bundled ? <span className="text-xs text-muted-foreground">{t("read-only")}</span> : null}
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          {subtitle ? <DialogDescription>{t(subtitle)}</DialogDescription> : null}
        </DialogHeader>
        ...
        <div className="max-h-72 overflow-y-auto rounded-md border border-border p-1">
          ...
          >
            ...
            <span className="min-w-0 flex-1 truncate">{t("Unfiled")}</span>
            {currentFolderId == null ? <span className="text-xs text-muted-foreground">{t("current")}</span> : null}
        ...
        <div className="min-h-5 text-xs text-muted-foreground">
          {previewPath ? (
            <span>
              {t("Moving to")} <span className="font-mono text-foreground">{previewPath}</span>
            </span>
          ) : (
            <span>{t("Pick a destination folder.")}</span>
          )}
        </div>
