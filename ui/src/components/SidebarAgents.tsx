...
function SidebarAgentItem({
  ...
  const trailingLabel = [
    ...
    hasInvalidOrgChain ? t("Invalid reporting chain") : null,
  ...
  const navItem = (
    <SidebarNavItem
      ...
      trailing={
        showBuiltInLifecycle || hasInvalidOrgChain ? (
          <span className="ml-1 flex shrink-0 items-center gap-1">
            ...
            {hasInvalidOrgChain ? (
              <AlertTriangle className="h-3.5 w-3.5 shrink-0 text-amber-500" aria-label={t("Invalid reporting chain")} />
            ) : null}
          </span>
        ) : undefined
      }
      ...
      liveAccessory={
        agent.pauseReason === "budget" ? <BudgetSidebarMarker title={t("Agent was paused by budget limits")} /> : undefined
      }
    />
  );
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          {onToggleStar ? (
            <>
              ...
              >
                ...
                <span>{starred ? t("Remove from starred") : t("Star agent")}</span>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
            </>
          ) : null}
          ...
          >
            ...
            <span>{leaving ? t("Leaving...") : t("Leave agent")}</span>
          </DropdownMenuItem>
        </DropdownMenuContent>
      </DropdownMenu>
    </div>
      ...
      {showSeeAllLink && (() => {
        ...
        const seeAllLink = (
          <Link
            ...
            aria-label={rail ? t("See all agents") : undefined}
            onClick={() => {
              if (isMobile) setSidebarOpen(false);
            }}
          ...
          >
            ...
            <span className={rail ? SIDEBAR_RAIL_HIDDEN_LABEL : undefined}>{t("See all agents")}</span>
          </Link>
        );
        return rail ? (
          <Tooltip>
            <TooltipTrigger asChild>{seeAllLink}</TooltipTrigger>
            <TooltipContent side="right">{t("See all agents")}</TooltipContent>
          </Tooltip>
        ) : (
          seeAllLink
        );
